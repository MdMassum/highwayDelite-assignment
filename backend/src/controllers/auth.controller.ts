...
export const requestSignupOtp = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { name, email, dob } = signupRequestSchema.parse(req.body);

    const existing = await User.findOne({ email });
    if (existing) return next(new ErrorHandler("User already exists", 400));
    ...
    await redis.setex(`signup:${email}`, 300, payload);
...
export const verifySignupOtp = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!data) return next(new ErrorHandler("OTP expired or not requested", 400));

    const { name, dob, hashedOtp } = JSON.parse(data);
    const hashedInput = crypto.createHash("sha256").update(otp).digest("hex");

    if (hashedOtp !== hashedInput) return next(new ErrorHandler("Invalid OTP", 400));
